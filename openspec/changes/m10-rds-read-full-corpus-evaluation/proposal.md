...
M7 has finished with 20,291 eligible evidence chunks synchronized to RDS, while the ECS application still reads a SQLite copy and the latest 60-question M9 report covered only 1,890 chunks. The next stage must make the private application read the complete RDS corpus through a reversible, read-only path and establish honest quality, latency, memory, and cost acceptance before any capacity increase or later product expansion.
...
- Add missing PostgreSQL schema parity and only the measured indexes required by bounded lexical, entity, relation, evidence-context, and vector reads.
...
- Measure cold and warm P50/P95 latency, bounded concurrency, peak application memory, connection usage, query plans, RDS wake/cold-start behavior, and observed RDS cost before changing indexes or capacity.
...
- `full-corpus-quality-evaluation`: Covers immutable corpus manifests, cross-backend evaluation, quality gates, performance and memory measurement, failure classification, and cost evidence.

### Modified Capabilities

- `cloud-database-validation`: Extends cloud validation from migration probes to production read-role, schema/index, query-plan, connection, and rollback acceptance.
...
- Operates on the retained same-VPC ECS, RDS, and SQLite snapshot. RDS remains capped at 4 RCU with automatic pause unless measurements and a separate approval justify a later capacity change.
